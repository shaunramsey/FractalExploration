...
b3 = np.linspace(lowerbound, upperbound, steps)
...
plt.title("2D slice of 3D Lyapunov Pulse Time Scale")
plt.xlabel("B1 value")
plt.ylabel("B2 value")
...
def animate(i):
    ...
    grid = getlyapexponent( (bb2, bb1, b3[i]) ) 
...
for i in range(len(b3)):
    grid = animate(i)
    subtitle = ax.text(0.5, 0.95, "B3: {}".format(b3[i]), bbox={'facecolor':'w', 'alpha':0.8, 'pad':5}, ha="center", transform=ax.transAxes,)
...
anim.save(r"Lyap_Pulse_3D_slice.gif", writer="imagemagick")
