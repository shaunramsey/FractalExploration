...
def binCalc(numBins, equations):
    ...
    for i in range(numBins):  # Loop to populate bins in binList with approperate contents
        ...
        for j in range(len(binList)):
            tempPercentage = binList[j].percentageOfRange(
...
# TODO Change naming from "fontStyle" to "guiFontStyle"
...
class slider:
    ...
    def makeSlider(self):
        ...
        self.sliderLabel = Label(
            self.frame, text=self.title, font=self.fontStyle)
        self.sliderLabel.pack()
        self.entry = Entry(
            self.frame, textvariable=self.callVar, font=self.fontStyle)
        self.entry.pack()
        self.slider = Scale(self.frame, from_=self.min, to=self.max, orient=HORIZONTAL,
                            variable=self.callVar, font=self.fontStyle, showvalue=0, length=200, resolution=self.precision)
        self.slider.pack()
...
# Converts integer range of precision (2-10 on the slider) to the decimal place equivalent
def calcPrecision(input):
    temp = "."
...
x0CallVar = DoubleVar()
x0CallVar.set(.5)
x0Slider = sliderInFrame(root=iniVarFrame, title="x0 Slider", min=0, max=1, default=.5,
                         fontStyle=fontStyle, precision=calcPrecision(precisionDefault), callVar=x0CallVar)
x0Slider.makeSlider()
...
rCallVar = DoubleVar()
rCallVar.set(3)
rSlider = sliderInFrame(root=iniVarFrame, title="r Slider", min=0, max=4, default=3,
                        fontStyle=fontStyle, precision=calcPrecision(precisionDefault), callVar=rCallVar)
rSlider.makeSlider()
...
logisticFrame = Frame(root, borderwidth=1,
                      highlightbackground="black", highlightthickness=1)
logisticOverrideFrame = Frame(logisticFrame)

logisticOverrideButton = Checkbutton(
    logisticOverrideFrame, text="Override With Logistic Points", variable=logisticOverrideOn, font=fontStyle)
logisticOverrideButton.pack(side=LEFT)
numLogisticPointSlider = sliderInFrame(root=logisticOverrideFrame, title="Number of Points on the Logistic Curve",
                                       min=3, max=500, default=5, fontStyle=fontStyle, precision=1, callVar=numLogisticPointCallVar)
...
binNumSlider = sliderInFrame(root=logisticFrame, title="Number of Bins", min=1,
                             max=50, default=2, fontStyle=fontStyle, precision=1, callVar=binNumCallVar)
    ...
    def addPoint(self, point):
        if self.duplicateCheck(point) == False:
            ...
            if logisticOverrideOn.get() == False:
                x0Slider.setRange(min=self.getPointByIndex(
                    0)[0], max=self.getPointByIndex(self.getLength()-1)[0])
            ...
            # TODO make this an error on the GUI
...
class equationRegister:
    ...
    class equation:
        ...
        def calcEQ(self):
            self.m = ((self.finalPoint[1] - self.initialPoint[1]) /
                      (self.finalPoint[0] - self.initialPoint[0]))
    ...
    def buildEquations(self, pointRegister):
        ...
        for i in range(pointRegister.getLength()-1):
            self.addEquation(pointRegister.getPointByIndex(i),
                             pointRegister.getPointByIndex(i+1))
    ...
    def getY(self, x):
        if x is None:
            print(
                "Error: Cobweb Plot can't calculate with given paramiters. Correct for 'tent' plot like format")
            return
        for i in range(self.getLength()):
            if self.isXInEQRange(x, self.equationList[i]) == True:
                return self.equationList[i].getY(x)
        # NOTE This should only call if the if statement is never true inside the previous for loop
        print("Error: X not in Register Range")
    ...
    def getSlope(self, x):
        ...
        # NOTE This should only call if the if statement is never true inside the previous for loop
        print("Error: X not in RRegister Range")
...
points.addPoint((2, 3))
points.addPoint((5, 0))
# Added because initializing the points sets x0 back to 0 when it should default at .5
x0CallVar.set(.5)
...
pointFrame = Frame(root, borderwidth=1,
                   highlightbackground="black", highlightthickness=1)
...
pointSelectionLabel = Label(
    pointDropdownFrame, text="Select Point:", font=fontStyle)
pointSelectionLabel.pack()


pointSelectionDropDown = OptionMenu(
    pointDropdownFrame, pointSelectionCallVar, *points.getPointStringList())  # NOTE Ignore warning
...
deletePointButton = Button(
    pointDeleteFrame, text="Delete Selected Point", command=deletePoint)
...
pointAddXSlider = sliderInFrame(root=pointAddFrame, title="X Value", min=0, max=10, default=0, fontStyle=fontStyle,
                                precision=calcPrecision(precisionDefault), callVar=pointAddXCallVar)  # FIXME What max?
...
pointAddYSlider = sliderInFrame(root=pointAddFrame, title="Y Value", min=0, max=10, default=0, fontStyle=fontStyle,
                                precision=calcPrecision(precisionDefault), callVar=pointAddYCallVar)  # FIXME What max?
...
def addPoint():
    ...
    for p in points.getPointStringList():
        pointSelectionDropDown["menu"].add_command(
            label=str(p), command=lambda pStr=p: pointSelectionCallVar.set(pStr))
    pointSelectionCallVar.set(str(newPoint))


addPointButton = Button(
    pointAddFrame, text="Add Specified Point", command=addPoint)
addPointButton.pack()
...
# Callback function for when precisionCallVar changes
def precisionChange(var, indx, mode):
...
# Tying the callback to the Variable
precisionCallVar.trace_add("write", precisionChange)

sliderPrecisionSlider = slider(title="Slider Precision", min=2, max=10, default=precisionDefault,
                               fontStyle=fontStyle, precision=1, row=3, column=4, callVar=precisionCallVar)
...
fontSlider = slider(title="Font Size", min=1, max=30, default=fontSizeDefault,
                    fontStyle=fontStyle, precision=1, row=4, column=4, callVar=fontSizeCallVar)
...
# Tying the callback to the Variable
fontSizeCallVar.trace_add("write", fontChange)
...
calcTimeLabelTitle = Label(
    calcTimeFrame, text="Lyapunov Calculation Time:", font=fontStyle)
calcTimeLabelTitle.pack()
calcTimeLabel = Label(
    calcTimeFrame, textvariable=calcTimeCallVar, font=fontStyle)
calcTimeLabel.pack()
calcTimeFrame.grid(row=5, column=0)

iterCalcTimeCallVar = StringVar()
iterCalcTimeFrame = Frame(root)
iterCalcTimeLabelTitle = Label(
    iterCalcTimeFrame, text="Iterative Lyapunov Calculation Time:", font=fontStyle)
iterCalcTimeLabelTitle.pack()
iterCalcTimeLabel = Label(
    iterCalcTimeFrame, textvariable=iterCalcTimeCallVar, font=fontStyle)
...
def calcLam(internalPoints):
    ...
    total_x_interval = internalPoints.getPointList(
    )[n-1][0] - internalPoints.getPointList()[0][0]  # total x _interval
    ...
    for i, p in enumerate(internalPoints.getPointList()):
        ...
        line_segment_x_proportions.append(
            (p[0] - last_point[0]) / total_x_interval)
        line_segment_slopes.append(
            (p[1] - last_point[1]) / (p[0] - last_point[0]))
        eqn_n = eqn_n + \
            str(line_segment_x_proportions[i-1]) + "*w_" + str(i) + " + "
    ...
    # print("---")
    ans_array = [1]
    # we need one less than the linesegments which is one less than the num points
    for i in range(n-2):
        ans_array.append(0)
...
# This was changed to a StringVar to enable error messages
iterLamCallVar = StringVar()
iterLamFrame = Frame(root)
iterLamLabelTitle = Label(
    iterLamFrame, text="Iterative Lyapunov Exponent:", font=fontStyle)
...
plotCobwebButton = Checkbutton(
    root, text="Plot Cobweb", variable=plotCobwebOn, font=fontStyle)
...
def calcLamLogistic(equations):
    ...
    for i in range(equations.getLength()):
        tempLambda = tempLambda + (equations.getEQByIndex(i).getWidth() *
                                   weightList[i] * np.log(np.abs(equations.getEQByIndex(i).getM())))
...
def plotLogistic():
    ...
    iterLamCallVar.set(calcIterLam(
        x0=x0CallVar.get(), equations=logisticEquations))
...
def plotting(i):
    ax1.clear()
    if logisticOverrideOn.get() == False:
        ax1.set_title("Tent Plot", fontname=fontStyle.actual(
            "family"), fontsize=(fontStyle.actual("size")-8))
        equations.buildEquations(points)
        for i in range(equations.getLength()):
            line = equations.getPointsByIndex(i)
            ax1.plot([line[0][0], line[1][0]], [
                     line[0][1], line[1][1]], linewidth=1)
        if plotCobwebOn.get() == True:
            plotCobweb(points, equations)
        lamCallVar.set(calcLam(points))
        iterLamCallVar.set(calcIterLam(
            x0=x0CallVar.get(), equations=equations))

    else:
        ax1.set_title("Logistic Equation Plot", fontname=fontStyle.actual(
            "family"), fontsize=(fontStyle.actual("size")-8))
        plotLogistic()

    ax1.set_xlabel("X Values", fontname=fontStyle.actual(
        "family"), fontsize=(fontStyle.actual("size")-10))
    ax1.set_ylabel("Y Values", fontname=fontStyle.actual(
        "family"), fontsize=(fontStyle.actual("size")-10))
    ax1.grid(True)
