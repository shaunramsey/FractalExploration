...
def calcLam():
    ...
    eqn_n = ""
    matrix_n = []
    n = len(points.getPointList()) #number of points
    total_x_interval = points.getPointList()[n-1][0] - points.getPointList()[0][0]  # total x _interval

    last_point = (0,0)
    for i,p in enumerate(points.getPointList()):
        if i == 0:
            last_point = p
            continue #skip the first point, line segments start at the second
        line_segment_x_intervals.append([last_point[0], p[0]])
