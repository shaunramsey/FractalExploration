...
abStringCallVar.set(abStringDefault)
def callback(input):
    for letter in input:
...
abEntry_label = Label(abEntry_frame, text="AB Entry", font=fontStyle)
abEntry_label.pack()
abEntry = Entry(abEntry_frame, textvariable=abStringCallVar, font = fontStyle, validate="all", validatecommand=(reg, '%P'))
abEntry.pack()
abEntry_frame.grid(row=4, column=0)
...
def calcLam(rAIn, rBIn, xIn, abStringIn):
    ...
    lam = 0
    abList = []
    for i in range(1, (totalLoopCount +1 )):
        if abString[(i % len(abString))] == "A":
            abList.append(rA)
        else:
            abList.append(rB)
