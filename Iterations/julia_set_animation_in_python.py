...
def animate(i):
    lowerbound = -1.2
    upperbound = 1.2
    steps = int((upperbound-lowerbound)/stepsize)
    c = .3 + i/2500 - 0.325j # this is the magic go from 0.3 to 0.5 on the reals over 100 iterations for our animation  
...
for i in range(500):
    print ("-"*20,"Step",i,"-"*20)
    grid = animate(i)
    im = plt.imshow(grid,cmap="terrain", origin = 'lower', extent=[lowerbound,upperbound,lowerbound,upperbound]) 
    ims.append([im])
