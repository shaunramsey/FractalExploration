...
x = np.arange(.5, 1, .001) #seed
hori_x_range = np.arange(.5, 1, .001) #starts from -.5001 to skip over 0

for i in range (1, 100): #throws away first 100 
    x = np.multiply(hori_x_range, (1 - (2 * abs(x- .5) ) ) ) 

for j in range(1,100): #goes through next 100 interations, where the orbit has settled down by now
